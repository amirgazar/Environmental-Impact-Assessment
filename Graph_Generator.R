...
ggsave(filename = "fig_4.svg", plot = plot_list[[1]], device = "svg")
ggsave(filename = "fig_5.svg", plot = plot_list[[2]], device = "svg")
...
ggsave(filename = "fig_7.svg", plot = p, device = "svg")
...
p<- ggplot(df.expert.5y, aes(x = PRICE_5y, y = INSTALLED_5y, size = EXPORTS_5y_pred, color = as.factor(INTERTIE_5y))) +
  ...
  labs(x = "PRICE",
       ...
       color = "INTERTIE") +
  scale_color_manual(values = palette)
ggsave(filename = "fig_8.svg", plot = p, device = "svg")
...
print(plot)
ggsave(filename = "fig_9.svg", plot = plot, device = "svg")
